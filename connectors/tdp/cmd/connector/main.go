...
import (
	...
	"github.com/data-power-io/noesis-connectors/connectors/tdp/internal/tdp"
...
func main() {
	...
	handler, err := tdp.NewHandler(logger)
	if err != nil {
		logger.Fatal("Failed to create TDP handler", zap.Error(err))
	}
	defer handler.Close()
	...
	logger.Info("TDP connector starting", zap.String("port", port))
