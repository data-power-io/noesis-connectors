...
// SendRecord sends a single record to the stream
func (rs *RecordStreamer) SendRecord(entity, key string, payload []byte, op connectorv1.Op, groupID string) error {
	rs.mutex.Lock()
	defer rs.mutex.Unlock()

	msg := &connectorv1.ReadMessage{
		Msg: &connectorv1.ReadMessage_Record{
			Record: &connectorv1.RecordMsg{
				Entity:   entity,
				Key:      key,
				Payload:  payload,
				SchemaId: rs.schemaID,
	...
	err := rs.stream.Send(msg)
	if err == nil {
		rs.recordsEmitted++
		rs.bytesEmitted += int64(len(payload))
		rs.lastEmitTime = time.Now()
	}

	return err
}

// SendRecordFromArrow converts an Arrow record to the streaming format and sends it
func (rs *RecordStreamer) SendRecordFromArrow(entity, keyField string, record arrow.Record, op connectorv1.Op, groupID string) error {
	// Find the key column
	keyColumnIndex := -1
	for i := 0; i < int(record.NumCols()); i++ {
		if record.Schema().Field(i).Name == keyField {
			keyColumnIndex = i
			break
		}
	}
	...
	for rowIdx := 0; rowIdx < int(record.NumRows()); rowIdx++ {
		// Extract key value
		keyArray := record.Column(keyColumnIndex)
		keyValue := rs.extractScalarValue(keyArray, rowIdx)
		...
		for colIdx := 0; colIdx < int(record.NumCols()); colIdx++ {
			...
			value := rs.extractScalarValue(column, rowIdx)
		...
		err = rs.SendRecord(entity, fmt.Sprintf("%v", keyValue), payload, op, groupID)
		if err != nil {
			return err
		}
	}
